...
const fade  = (delay) => ({ initial: { opacity: 0 }, animate: { opacity: 1 }, transition: { duration: 0.8, delay } })
const slide = (delay) => ({ initial: { opacity: 0, y: 22 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.8, delay } })
...
export default function LandingPage() {
  ...
  const enter = () => guest ? navigate('/album') : setShowModal(true)

  return (
    <div
      className="paper-texture"
      style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }}
    >
      {/* ── Corner flowers ── */}
      ...
      {/* ── Content ── */}
      <div style={{ position: 'relative', zIndex: 2, textAlign: 'center', padding: '2rem 1.5rem', maxWidth: '680px', width: '100%' }}>
        ...
        <motion.p {...fade(0.2)} style={{
          ...
          color: '#B8A99A', marginBottom: '0.75rem'
        }}>
        ...
        <motion.div {...fade(0.48)} style={{ marginTop: '1.25rem' }}>
          <div className="divider-floral" style={{ maxWidth: '340px', margin: '0 auto' }}>
            <span style={{ fontFamily: '"Playfair Display", serif', fontStyle: 'italic', fontSize: '1.15rem', color: '#6B5757' }}>
        ...
        {/* Footnote */}
        <motion.p {...fade(0.88)} style={{
          fontFamily: 'Lato, sans-serif', fontSize: '0.75rem',
          color: '#9A8A8A', marginTop: '1.5rem'
        }}>
      ...
      {/* ── Side accent flowers ── */}
      <div className="animate-float-slow" style={{ position: 'absolute', left: '2%', top: '38%', opacity: 0.18, pointerEvents: 'none' }}>
        <FlowerIcon size={46} color="#F8E7A1" />
      </div>
      <div className="animate-float" style={{ position: 'absolute', right: '3%', top: '58%', opacity: 0.14, pointerEvents: 'none' }}>
